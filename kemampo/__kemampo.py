...
class Kemampo:
    ...
    def create_controller(self, target_model: DeclarativeMeta):
        self.__log("Instantiating Generic Controller")                                      # Instantiate Generic Controller for table provided
        gc = _GenericController(target_model)                                               # |

        setattr(gc, "__log", self.__log)                                                    # add logger to Generic Controller
        self.__log("Setting Session Generator for Generic Controller")                      # Add Session generator to Generic Controller
        setattr(gc, "session_generator", self.session_generator)                            # |
        self.__log(f"Setting Controller classname to {target_model.__name__}Controller")    # Setting Generic Controller classname
        setattr(gc.__class__, "__name__", f"{target_model.__name__}Controller")             # |
        self.__log(f"Setting Controller name to {target_model.__name__}Controller")         # Setting Generic Controller name
        setattr(gc, "__name__", f"{target_model.__name__}Controller")                       # |
